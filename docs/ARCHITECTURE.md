...
The fifty-four themes are distributed across: warm dark (Eventide, Ember, Cordovan, Tungsten, Nocturne, Tallow, Bamboo, Carmine, Rosewater, Madder, Espresso, Cinder, Kiln, Noir), cool dark (Aether, Fjord, Umbra, Cobalt, Ironcast, Sumi, Synthwave, Dusk, Boreal, Lapis), green dark (Absinthe, Verdigris, Petrichor, Canopy, Verdure, Phosphor, Lichen), teal dark (Nacreous, Basalt), neutral dark (Sable, Graphite, Cast, Weld), purple dark (Amethyst, Iris, Damson), and light (Parchment, Daybook, Cirrus, Solano, Saffron, Ochre, Bloom, Fern, Stark, Brume, Wisteria, Seafoam, Apricot, Overcast). A new theme must occupy a hue territory not already represented, or a substantially different atmospheric concept within the same hue region. The hue territory table in `CONTRIBUTING.md` is authoritative for human evaluation.

**Automated uniqueness check:** `scripts/__tests__/validate-themes.test.js` enforces palette uniqueness using OKLab perceptual color distance (ΔE). OKLab is perceptually uniform — it captures saturation and lightness differences that hue-angle checks miss. For each pair of same-type themes, the test computes ΔE between their `primary`, `accent`, and `muted` tokens. A pair fails if **all three** token distances fall below `MIN_DIST = 0.07` (≈ 3.5 just-noticeable-differences). A theme that shares a primary hue territory but uses a distinct muted color (for example, Eventide's lavender muted vs Ember's warm brown muted) passes naturally without a grandfathered exception. Two legacy pairs that pre-date this rule are in a `KNOWN_CLOSE` set with the weaker theme (lower WCAG minimum) identified in a comment. The full rationale and threshold derivation are in `docs/decisions/ADR-008.md`.
